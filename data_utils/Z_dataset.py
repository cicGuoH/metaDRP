...
class DatasetForMetaTrain(Dataset):
    def __init__(self, dataset_args, dataset_name="maml_train"):
        ...
        self.num_support_ccls = dataset_args.num_support_ccls
        self.num_query_ccls = dataset_args.num_query_ccls
    ...
    def get_set(self, drug_subset, rnd_seed):
        ...
        sample_idx = rng.choice([i for i in range(len(drug_subset))], self.num_support_ccls+self.num_query_ccls)
        drug_id = drug_subset["Drug_ID"][0]
        support_df = drug_subset.iloc[sample_idx[:self.num_support_ccls], :]
        query_df = drug_subset.iloc[sample_idx[self.num_support_ccls:], :]
        ...
        query_y = torch.tensor(query_df["IC50"].values.tolist())
        support_ccl_fea = torch.stack(support_ccl_fea, dim=0)
        query_ccl_fea = torch.stack(query_ccl_fea, dim=0)
        
        support_drug_fea = collator_forDrug([drug_fea for i in range(self.num_support_ccls)])
        query_drug_fea = collator_forDrug([drug_fea for i in range(self.num_query_ccls)])
...
class DatasetForFewShot(Dataset):
    def __init__(self, dataset_args, dataset_name="maml_test"):
        ...
        self.num_support_ccls = dataset_args.val_num_support_ccls
        self.val_samples_per_batch = dataset_args.val_samples_per_batch
        self.mini_batch = dataset_args.mini_batch
        
        drug_tissue_info = pd.read_csv(osp.join(self.dr_data_dir, "%s.csv" % dataset_name))
        self.drug_tissue_info = drug_tissue_info[drug_tissue_info["Size"] >= self.num_support_ccls + 1]
    ...
    def get_set_minibatch(self, drug_subset, rnd_seed, ccl_graph=True):
        ...
        tmp_num_mini_batch = len(sample_idx[self.num_support_ccls:]) // self.val_samples_per_batch
        num_mini_batch =  tmp_num_mini_batch if len(sample_idx[self.num_support_ccls:]) % self.val_samples_per_batch == 0 else (tmp_num_mini_batch + 1)
        ...
        for i in range(num_mini_batch):
            start_ind = self.num_support_ccls + i * self.val_samples_per_batch
            end_ind = min(self.num_support_ccls + (i+1) * self.val_samples_per_batch, len(sample_idx))
            if (len(sample_idx) - end_ind) <= 4:
                end_ind = len(sample_idx)
            query_df = drug_subset.iloc[start_ind:end_ind, :]
            query_ccl_fea = [self.ccl_dict[ccl_item] for ccl_item in query_df[self.sample_name]]
            query_ccl_fea = torch.stack(query_ccl_fea, dim=0)
            query_y = torch.tensor(query_df["IC50"].values.tolist())
    ...
    def get_set(self, drug_subset, rnd_seed, ccl_graph=True):
        rng = np.random.RandomState(rnd_seed)
        sample_idx = [i for i in range(len(drug_subset))]
        rng.shuffle(sample_idx)
    ...
    def __getitem__(self, index):
        ...
        if self.mini_batch:
            ...
            support_drug_fea, support_ccl_fea, support_y, query_drug_fea, query_ccl_fea, query_y = self.get_set(drug_subset=drug_subset, 
                                                                                                                rnd_seed=self.seed)
            return support_drug_fea, support_ccl_fea, support_y, query_drug_fea, query_ccl_fea, query_y
...
class DatasetForZeroShot(Dataset):
    ...
    def get_set_minibatch(self, drug_subset, rnd_seed, ccl_graph=True):
        ...
        sample_idx = [i for i in range(len(drug_subset))]
        rng.shuffle(sample_idx)
        drug_subset = drug_subset.iloc[sample_idx, :]
    ...
    def get_set(self, drug_subset, rnd_seed, ccl_graph=True):
        ...
        drug_fea = self.drug_dict[drug_id]
        query_ccl_fea = [self.ccl_dict[ccl_item] for ccl_item in drug_subset[self.sample_name]]
        query_y = torch.tensor(drug_subset["IC50"].values.tolist())
        query_drug_fea = collator_forDrug([drug_fea for i in range(len(sample_idx))])
        return query_drug_fea, query_ccl_fea, query_y
    ...
    def __getitem__(self, index):
        ...
        if self.mini_batch:
            ...
            query_drug_fea, query_ccl_fea, query_y = self.get_set(drug_subset=drug_subset, rnd_seed=self.seed)
            return query_drug_fea, query_ccl_fea, query_y
...
class DRDataloderForFewShotLearning:
    ...
    def get_dataloader(self, dataset):
        dataloader = DataLoader(dataset, batch_size=self.num_batches_per_iter, collate_fn=collate_fn_meta, 
                      shuffle=True, num_workers=self.num_workers, drop_last=True) # shuffle=False is OK?
        return dataloader
