...
def main():
    ...
    tar_file_path = osp.join("../model_visual", args.drug_dataset_name, "%s-%s.csv" % (drug_id, tissue_id))
    if args.k > 0:
        args.saved_path = osp.join("../model_visual", args.drug_dataset_name, "%s_%s_wFT" % (drug_id, tissue_id))
    else:
        args.saved_path = osp.join("../model_visual", args.drug_dataset_name, "%s_%s_woFT" % (drug_id, tissue_id))
    ...
    if args.k > 0 :
        explainer.crox_viz_samples(drug_id, tar_ccls, k=args.k, rnd_seed=args.rnd_seed, src_drug_id=src_drug_id, src_tissue_id=tissue_id)
    else:
        explainer.viz_samples(drug_id=drug_id, tissue_id=tissue_id, tar_ccls=tar_ccls, 
