...
def arg_parse():
    ...
    parser.add_argument('--drug_id', type=int, default=45)
    parser.add_argument('--src_drug_id', type=int, default=45)
    parser.add_argument('--tissue_id', type=int, default=7)
    # dataset
    parser.add_argument('--drug_dataset_name', type=str, default="TCGA", help="directory path of drug")
    parser.add_argument('--ccl_dataset_name', type=str, default="TCGA", help="directory path of ccl")
    ...
    parser.add_argument('--sample_name', type=str, default="Sample_ID")
...
def main():
    ...
    args.dr_data_dir = osp.join("../model_explain", args.drug_dataset_name)
    ...
    src_drug_id = "drug_%d" % args.src_drug_id
        
    tar_file_path = osp.join("../model_explain", args.drug_dataset_name, "%s-%s.csv" % (drug_id, tissue_id))
    if args.k > 0:
    ...
    for rnd_seed in rnd_seeds:
        if args.k > 0 :
            explainer.crox_explain_samples(drug_id, tar_ccls, k=args.k, rnd_seed=rnd_seed, src_drug_id=src_drug_id, src_tissue_id=tissue_id)
        else:
